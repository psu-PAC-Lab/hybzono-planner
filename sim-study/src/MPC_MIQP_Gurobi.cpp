...
// destructor
MPC_MIQP_Gurobi::~MPC_MIQP_Gurobi()
{
    delete [] x_gur;
    delete model_ptr;
}
...
// setup
void MPC_MIQP_Gurobi::set_MI_settings(const Gurobi_Settings &settings)
{
    gurobi_settings = settings;
}

void MPC_MIQP_Gurobi::build_controller()
{
    MPC_MIQP::build_controller();
    make_full_system_matrices();
    set_gurobi_settings();
}

void MPC_MIQP_Gurobi::set_gurobi_settings()
{
...
void MPC_MIQP_Gurobi::set_terminal_hybzono_constraints(const ZonoCpp::HybZono &X_hz_term, const Eigen::Ref<const Eigen::VectorXd> term_region_cost_vec,
...
int MPC_MIQP_Gurobi::get_terminal_region_selection()
{
    throw std::runtime_error("Terminal hybzono constraints not currently supported using Gurobi as MIQP solver");
}

// control methods (LTI)
std::pair<Eigen::VectorXd, bool> MPC_MIQP_Gurobi::control(const Eigen::Ref<const Eigen::VectorXd> x, const std::vector<Eigen::VectorXd> &x_ref)
{
    // set x0 and x_ref
    this->x0 = x;
    this->x_ref = x_ref;
...
std::pair<Eigen::VectorXd, bool> MPC_MIQP_Gurobi::control(const Eigen::Ref<const Eigen::VectorXd> x)
{
    // set x_ref
...
void MPC_MIQP_Gurobi::make_full_system_matrices()
{
    // declare
...
void MPC_MIQP_Gurobi::make_optim_problem()
{
...
bool MPC_MIQP_Gurobi::solve_optimization_problem()
{
    // solve and time
...
// utilities
void MPC_MIQP_Gurobi::quad_cost(GRBModel * model_ptr, const GRBVar *x, 
...
void MPC_MIQP_Gurobi::lin_cons(GRBModel * model_ptr, const GRBVar *x, 
